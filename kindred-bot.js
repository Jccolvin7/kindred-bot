...
function pickArchetype(seed, ratings) {
  const allTitles = ratings.map(r => r.item_name.toLowerCase());
  let genre = 'Genre-Blending';
  if (allTitles.some(t => ['interstellar','blade runner','dune','arrival','ex machina','inception'].some(k => t.includes(k)))) genre = 'Sci-Fi';
  else if (ratings.filter(r => r.category === 'games' && r.rating >= 4).length >= 2) genre = 'Narrative';
  else if (ratings.filter(r => r.category === 'books' && r.rating >= 4).length >= 2) genre = 'Literary';
  let hash = 0;
...
client.on('interactionCreate', async interaction => {
  ...
  try {
    ...
    if (commandName === 'passport') {
      ...
      const archetype = pickArchetype(user.username, ratings);
      const fresh = getFreshness(ratings.length);
      ...
      const embed = new EmbedBuilder()
        .setColor(PURPLE)
        .setTitle(`🪪 ${archetype}`)
        .setDescription(`**Level:** ${level}\n**${ratings.length} items rated** across film, games, and books`)
        .addFields(
          { name: '🎬 Film & TV', value: `${counts.film} rated`, inline: true },
          { name: '🎮 Games', value: `${counts.games} rated`, inline: true },
          { name: '📚 Books', value: `${counts.books} rated`, inline: true },
          { name: '🔄 Freshness', value: fresh.remaining === 0 ? '100% — fresh!' : `${fresh.pct}% — rate ${fresh.remaining} more to refresh`, inline: false },
        )
        .setFooter({ text: 'Kindred Taste Passport · /rate to keep building it' });
